...
export default function ChatbotSearch() {
  const [messages, setMessages] = useState([
    { role: 'bot', content: 'Hello! I can help you find activities based on your group and interests. What would you like to do?' }
  ]);
  ...
  const activities = ['Outdoor', 'Indoor', 'Sports', 'Cultural', 'Relaxation', 'Adventure'];
  ...
  const handleSearch = () => {
    ...
    searchResults.forEach(result => {
      new google.maps.Marker({
        position: { lat: 40.730610, lng: -73.935242 }, // Replace with real location data
        map: mapInstance.current,
        title: result.title,
      });
    });
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="p-4 border-b border-base-300">
              ...
              <div className="mb-4">
                ...
                />
                <button onClick={handleAreaSearch} className="btn btn-primary w-full mb-4">
                  Search Area
                </button>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {activities.map(activity => (
                    <button
                      key={activity}
                      className={`btn btn-sm ${selectedActivities.includes(activity) ? 'btn-active' : 'btn-outline'}`}
                ...
                {searchResults.map((result, index) => (
                  <div key={index} className="card bg-base-200 shadow-sm mb-2">
                    <div className="card-body p-4">
                      ...
                      <p className="text-xs">
                        <span className="badge badge-outline mr-2">{result.type}</span>
                        <span className="badge badge-outline">{result.group}</span>
                      </p>
                    </div>
                  </div>
                ))}
                  ...
                  <div key={index} className={`chat ${message.role === 'user' ? 'chat-end' : 'chat-start'}`}>
                    <div className="chat-image avatar">
                      <div className="w-10 rounded-full">
                        <img src={message.role === 'user' ? '/placeholder.svg' : '/placeholder.svg'} alt={message.role} />
                      </div>
                    </div>
